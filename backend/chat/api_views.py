...
class UserView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if not term:
            return (
                User.objects.all().exclude(id=self.request.user.id).order_by("username")
            )
        return (
...
class ChatMessagesView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if not is_valid_uuid(roomId):
            raise ValidationError({"roomId": ["Invalid room UUID"]})
        if not ChatRoom.objects.filter(id=roomId, members=user).exists():
            raise ValidationError({"roomId": ["Room does not exist"]})
...
class ChatRoomView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return ChatRoom.objects.filter(members=user, name__icontains=term).order_by(
            "name"
        )
