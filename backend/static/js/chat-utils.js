...
const topRoomInformation = (chatRoom, currentUser = USERID) => {
  ...
  //     members.push(`<span class="font-bold text-sm text-blue-500">@saya</span>`);
  //   } else {
...
const memberList = (chatRoom, currentUser = USERID) => {
  ...
  roomObj.all_members.forEach(member => {
    let profileInitial = chatRoom.getUserNameById(member.id).charAt(0).toUpperCase();
    if (member.id == currentUser) {
      members.push(`
        <li class="member-item">
          <div class="profile-circle bg-blue-500">Y</div>
      ...
      members.push(`
        <li class="member-item">
          <div class="profile-circle ${chatRoom.memberTxtColor[member.id]}">${profileInitial}</div>
          <span class="member-name">${chatRoom.getUserNameById(member.id)}</span>
        </li>
      `);
    }
  });
  ...
  return `
    <div class="flex flex-col">
      <div class="block w-full text-left mb-2">
...
const incomingChatTemplate = (chatObj, userName, textColor) => {
  return `
        <li class="flex justify-start">
          <div>
          <span class="block text-left ${textColor} font-bold text-sm">${userName}</span>
...
const sendChatTemplate = (chatObj) => {
  return `
      ...
      <div>
        <span class="block text-right font-bold text-blue-500 text-sm">You</span>
        <div class="relative max-w-xl px-4 py-2 text-gray-700 bg-white rounded shadow">
          <p class="text-slate-400 text-xs text-time" data-time="${chatObj.created}">${compareTime(chatObj.created)}</p>
          <span class="block">${chatObj.message}</span>
        </div>
...
const roomTemplate = (roomObj, colorClass) => {
  return `
    ...
    <div class="w-full pb-2">
      <div class="flex justify-between">
        <span class="block ml-2 font-semibold text-gray-600">${displayShortMsg(roomObj.name, 15)
    }</span>
...
function toggleRoomBlock() {
  ...
  if ($(".room-container-block").hasClass("hidden")) {
    ...
    $("#kanan").removeClass("md:col-span-12");
    $("#kanan").addClass("md:col-span-8");
